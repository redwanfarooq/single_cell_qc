...
unlockBinding("params", .GlobalEnv)
for (x in c("outdir", "features_matrix", "hto_matrix", "fragments")) {
...
for (modality in modalities[modalities %in% params$cell_calling_modalities]) {
  plot <- ggplotly(
    bcrank.plot(
      bcrank.res[[modality]],
      cells = bc.cells,
      title = paste(toupper(modality), "barcode rank plot"),
      x.lab = "Rank",
...
bc.cells <- demux.res %>%
  ...
  rownames()
features.mat <- lapply(features.mat, function(x) x[, bc.cells])
cols <- sapply(seq_along(wl), function(x) scales::brewer_pal(palette = "Greens", direction = -1)(length(wl))[x]) %>% setNames(wl)
...
```{r hto-demultiplexing-raw-counts, eval=!is.null(params$hto_matrix)}
cat("### Raw HTO counts", "\n\n")
ggplotly(
  logcounts.plot(
    matrix = hto.mat,
    features.pattern = "^HTO_",
    title = "Histogram of HTO counts",
    x.lab = "UMI count",
    fill.lab = "HTO",
    binwidth = 0.05,
    alpha = 0.75
...
```{r hto-cross-sensitivities, eval=!is.null(params$hto_matrix)}
cat("### Cross-sensitivities", "\n\n")
...
known <- if (exists("demux.res")) demux.res[colnames(features.mat[[1]]), "Type"] == "multiplet" else rep(FALSE, times = ncol(features.mat[[1]]))
