...
if (exists("demux.res")) {
  metadata$hash_id <- sapply(demux.res[metadata$barcode, ]$HTO, function(x) md[md$hto == x, "hash_id", drop = TRUE])
}
metadata <- left_join(metadata, md %>% select(-c(hto, cells_loaded, hash_id)) %>% distinct(), by = "sample_id")
