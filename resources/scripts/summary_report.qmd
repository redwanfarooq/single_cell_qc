...
density function per hashtag (measure of signal-to-noise ratio; ideal >0.99, acceptable >0.9)
:::
"""
    )
```
...
if all(os.path.isfile(files[1]) for files in files["droplet_qc"].values()):
    ...
    df_signal_strength = (
        pd.melt(df_signal_strength, id_vars="Sample", value_vars=cols[1:], var_name="HTO", value_name="Signal Strength")
        .assign(Status=lambda x: ["pass" if _ > 0.9 else "borderline" if _ > 0.8 else "fail" for _ in x["Signal Strength"]])
    )
    ...
    fig_hto_signal = (
        px.strip(
            ...
            hover_data="Sample",
            color="Status",
            color_discrete_sequence=list(COLOURS.values()),
            stripmode="overlay",
            range_x=[0.5, 1],
...
if all(os.path.isfile(files[1]) for files in files["droplet_qc"].values()):
    metrics.append("**Assignment** number of cell barcodes passing all library-level QC filters assigned to each hashed sample")
