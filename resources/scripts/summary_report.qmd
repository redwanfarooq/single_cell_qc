...
if all(os.path.isfile(files[1]) for files in files["droplet_qc"].values()):
    ...
    df_signal_strength = reduce(lambda x, y: pd.merge(x, y, how="outer"), df_signal_strength)
    ...
    df_assignment = reduce(lambda x, y: pd.merge(x, y, how="outer"), df_assignment).convert_dtypes()
