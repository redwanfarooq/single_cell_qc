...
droplet.qc.res <- vroom::vroom(
  ...
  mutate(sample_id = if (exists(hash_id)) factor(hash_id, levels = sort(unique(hash_id)), ordered = TRUE) else factor(sample_id, levels = sort(unique(sample_id)), ordered = TRUE)) %>%
  as.data.frame()
