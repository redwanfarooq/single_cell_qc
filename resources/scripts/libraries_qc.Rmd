...
```{r gex-qc-library-size, eval=all(!is.null(features.mat[["Gene Expression"]]), as.logical(params$gex_filters$library_size))}
cat("### Library size", "\n\n")
ggplotly(
  violin.plot(
    colData(sce),
...
```{r gex-qc-library-complexity, eval=all(!is.null(features.mat[["Gene Expression"]]), as.logical(params$gex_filters$library_complexity))}
...
```{r gex-qc-mitochondrial-fraction, eval=all(!is.null(features.mat[["Gene Expression"]]), as.logical(params$gex_filters$mitochondrial_fraction))}
...
if (as.logical(params$atac_filters$peak_count)) {
  seu$sum <- colSums(seu)
  qc$low_peak_count <- if (is.logical(params$atac_filters$peak_count)) isOutlier(seu$sum, type = "lower", log = TRUE) else seu$sum < params$atac_filters$peak_count
}
if (as.logical(params$atac_filters$peak_fraction)) {
  ...
  qc$low_peak_fraction <- if (is.logical(params$atac_filters$peak_fraction)) isOutlier(seu$FRiP, type = "lower", log = FALSE) else seu$FRiP < params$atac_filters$peak_fraction
}
if (as.logical(params$atac_filters$blacklist_fraction)) {
  ...
  qc$high_blacklist_fraction <- if (is.logical(params$atac_filters$blacklist_fraction)) isOutlier(seu$blacklist_fraction, type = "higher", log = FALSE, nmads = 8) else seu$blacklist_fraction > params$atac_filters$blacklist_fraction
}
if (as.logical(params$atac_filters$nucleosome_signal)) {
  seu <- NucleosomeSignal(seu)
  qc$high_nucleosome_signal <- if (is.logical(params$atac_filters$nucleosome_signal)) isOutlier(seu$nucleosome_signal, type = "higher", log = FALSE, nmads = 8) else seu$nucleosome_signal > params$atac_filters$nucleosome_signal
}
if (as.logical(params$atac_filters$tss_enrichment)) {
  seu <- TSSEnrichment(seu, fast = FALSE)
  qc$low_tss_enrichment <- if (is.logical(params$atac_filters$tss_enrichment)) isOutlier(seu$TSS.enrichment, type = "lower", log = TRUE) else seu$TSS.enrichment < params$atac_filters$tss_enrichment
}
qc$discard <- combine.filters(qc)
...
```{r atac-qc-peak-count, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$peak_count))}
cat("### Peak count", "\n\n")
ggplotly(
  violin.plot(
    seu[[]],
...
```{r atac-qc-peak-fraction, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$peak_fraction))}
cat("### Peak fraction", "\n\n")
ggplotly(
  violin.plot(
    seu[[]],
...
```{r atac-qc-blacklist-fraction, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$blacklist_fraction))}
...
```{r atac-qc-nucleosome-signal, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$nucleosome_signal))}
...
```{r atac-qc-tss-enrichment, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$tss_enrichment))}
cat("### TSS enrichment", "\n\n")
ggplotly(
  violin.plot(
    seu[[]],
