...
```{r atac-qc-fragment-count, eval=all(!is.null(features.mat[["Peaks"]]), as.logical(params$atac_filters$fragment_count))}
cat("### Fragment count", "\n\n")
ggplotly(
  violin.plot(
    seu[[]],
...
```{r atac-qc-diagnostics, fig.width=7, fig.height=5, eval=!is.null(features.mat[["Peaks"]])}
cat("### Diagnostics", "\n\n")
...
scatter.plot(
  ...
  ggplot2::scale_colour_manual(values = c("grey80", "#d82526"))
cat("\n\n")
TSSPlot(subset(seu, cells = WhichCells(seu, expression = discard, invert = TRUE))) +
  scale_colour_manual(values = "#595959") + 
  theme_minimal() +
  theme(
...
cat("\n\n")
FragmentHistogram(seu, cells = WhichCells(seu, expression = discard, invert = TRUE)) +
