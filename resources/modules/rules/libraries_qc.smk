...
rule libraries_qc:
    ...
    params:
        ...
        droplet_qc = lambda wildcards: get_path(wildcards, os.path.join(config["output_dir"]["data"], "droplet_qc/{sample}.csv")),
        output = lambda wildcards: get_path(wildcards, os.path.join(config["output_dir"]["data"], "libraries_qc/{sample}.csv")),
        gex_matrix = lambda wildcards: get_path(wildcards, config.get("gex_matrix", None)),
        gex_filters = config.get("gex_filters", None),
        atac_matrix = lambda wildcards: get_path(wildcards, config.get("atac_matrix", None)),
        atac_filters = config.get("atac_filters", None),
        adt_matrix = lambda wildcards: get_path(wildcards, config.get("adt_matrix", None)),
