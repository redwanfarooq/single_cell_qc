...
unlockBinding("params", .GlobalEnv)
for (x in c("outdir", "gex_matrix", "hto_matrix")) {
...
5. Output TSV file containing cell metadata for barcodes passing QC filters and text file containing list of empty droplet barcodes
...
set.seed(100)
ed.res <- emptyDrops(gex.mat, by.rank = sum(sapply(md, function(x) x$cells_loaded)), niters = params$emptydrops_niters, test.ambient = TRUE)
ed.stats <- list(
  cells = sum(ed.res$FDR < 0.001, na.rm = TRUE),
  empty = sum(ed.res$FDR >= 0.001, na.rm = TRUE),
  background = sum(is.na(ed.res$FDR)),
  limited = any(ed.res$Limited[which(ed.res$FDR >= 0.001)])
)
bcrank.res <- barcodeRanks(gex.mat)
bc.empty <- gex.mat[, which(ed.res$FDR >= 0.001 | is.na(ed.res$FDR))] %>% colnames()
...
```{r output}
metadata <- colData(sce)[!sce$known & !sce$predicted, "Sample", drop = FALSE] %>%
  as.data.frame() %>%
