...
if (params$hto_matrix == params$gex_matrix) {
  ...
  hto.mat <- get.barcounter.matrix(params$hto_matrix, features.pattern = "^HTO_", add.suffix = TRUE)
}
wl <- names(md)
...
```{r hto-demultiplexing-raw-hto-counts, fig.width=7, fig.height=length(wl)}
cat("### Raw HTO counts", "\n\n")
logcounts.plot(hto.mat, title = "Raw HTO counts")
```

```{r hto-demultiplexing-demultiplexed-hto-counts, fig.width=7, fig.height=3*ceiling(length(wl)/2)}
...
cat("\n\n")
multiplet.plot(sce, rd.res, projection = params$multiplet_projection, random.seed = 100) %>% ggplotly(tooltip = "colour")
```
