...
1. Load count matrices and subset to common barcodes that passed droplet processing QC
2. Calculate QC metrics and discard outliers on 1 or more metrics
3. Output text file containing list of barcodes passing QC filters
...
if (params$atac_filters$blacklist_fraction) {
  ...
  qc$high_blacklist_fraction <- isOutlier(seu$blacklist_fraction, type = "higher", log = FALSE, nmads = 10) # only exclude extreme outliers
}
if (params$atac_filters$nucleosome_signal) {
  seu <- NucleosomeSignal(seu)
  qc$high_nucleosome_signal <- isOutlier(seu$nucleosome_signal, type = "higher", log = FALSE, nmads = 10) # only exclude extreme outliers
}
